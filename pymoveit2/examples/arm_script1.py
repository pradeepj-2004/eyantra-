...
class move_pose(Node):
    ...
    def servo_circular_motion(self,speed_x,speed_y,speed_z,l):
        ...
        while i<l:
            ...
            self.twist_msg.twist.linear.x = speed_x*3.0
            self.twist_msg.twist.linear.y = speed_y*3.0
            self.twist_msg.twist.linear.z = speed_z*3.0
...
def main(args=None):
    ...
    for j in range(3):  
        ...
        while True:
            ...
            future=main_object.wait_for_callback_result()
            if j==0:
                if (math.sqrt(((trav[0]-future[0])**2)+((trav[1]-future[1])**2)+(trav[1]-future[1])**2)<0.40):   #Servoing backwards to avoid collision of box with rack    
                    main_object.servo_circular_motion(0.0,0.0,-0.1,1)                    
                else:
                    print('back servo done')
                    break
            else:
                if (math.sqrt(((trav[0]-future[0])**2)+((trav[1]-future[1])**2)+(trav[1]-future[1])**2)<0.30):   #Servoing backwards to avoid collision of box with rack    
                    main_object.servo_circular_motion(0.0,0.0,-0.1,1)                    
                else:
                    print('back servo done')
                    break
